...
if (
    ...
    !empty($_POST["new_password_retype"])
) {
    // XSS
    ...
    $check_password = $bdd->prepare(
        "SELECT password FROM effectif WHERE token = :token"
    );
    ...
    if (password_verify($current_password, $data_password["password"])) {
        ...
        if ($new_password === $new_password_retype) {
            ...
            $update = $bdd->prepare(
                "UPDATE effectif SET password = :password WHERE token = :token"
            );
            $update->execute([
